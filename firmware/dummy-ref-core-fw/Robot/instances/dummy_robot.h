...
public:
    ...
    const DOF6Kinematic::Joint6D_t REST_POSE = {0, -70, 178, 0, 0, 0};
    ...
    const float DEFAULT_JOINT_ACCELERATION_LOW = 8;    // 0~100
    const float DEFAULT_JOINT_ACCELERATION_HIGH = 80;  // 0~100
    ...
    std::string GetIP();
    void SetWiFiMode(std::string mode);
    std::string GetWifiMode();
    void Heartbeat();
    void SetAutoSendPos(bool flag);
    bool isSendPos();
    void SendPosJ();
