...
void DummyRobot::SetIP(std::string ip) {
    this->ip = ip;
}
std::string DummyRobot::GetIP() {
    return this->ip;
}

void DummyRobot::SetWiFiMode(std::string wifi_mode) {
    this->wifiMode = wifi_mode;
}
std::string DummyRobot::GetWifiMode() {
    return this->wifiMode;
}

void DummyRobot::SetAutoSendPos(bool flag) {
    this->autoSendPos = flag;
}

bool DummyRobot::isSendPos() {
    return this->autoSendPos;
}

void DummyRobot::SendPosJ() {
    Respond(*uart4StreamOutputPtr,"pos %.1f %.1f %.1f %.1f %.1f %.1f",
           currentJoints.a[0], currentJoints.a[1], currentJoints.a[2],
           currentJoints.a[3], currentJoints.a[4], currentJoints.a[5]);
}
    ...
    hcan(_hcan)
{
    ...
    motorJ[2] = new CtrlStepMotor(_hcan, 2, false, 80, -75, 90);
    motorJ[3] = new CtrlStepMotor(_hcan, 3, false, 50, -35, 178);
...
void DummyRobot::Homing()
{
    ...
    osDelay(100);
    Respond(*uart4StreamOutputPtr,"pos %.2f %.2f %.2f %.2f %.2f %.2f",
               currentJoints.a[0], currentJoints.a[1], currentJoints.a[2],
               currentJoints.a[3], currentJoints.a[4], currentJoints.a[5]);
    SetJointSpeed(lastSpeed);
}
void DummyRobot::Heartbeat() {
    Respond(*uart4StreamOutputPtr,"heartbeat");
}
    ...
    switch (context->commandMode)
    {
        ...
        case COMMAND_CONTINUES_TRAJECTORY:
            if (_cmd[0] == '>' || _cmd[0] == '&')
            {
                ...
                Respond(*usbStreamOutputPtr, "MoveJ ok");
                Respond(*uart4StreamOutputPtr, "MoveJ ok");
            } else if (_cmd[0] == '@')
            {
                float pose[6];
                float speed;
                ...
                Respond(*usbStreamOutputPtr, "MoveL ok");
                Respond(*uart4StreamOutputPtr, "MoveL ok");
            }
