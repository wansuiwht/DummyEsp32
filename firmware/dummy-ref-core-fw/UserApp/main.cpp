...
void ThreadOledUpdate(void* argument)
{
    ...
    for (;;)
    {
        ...
        oled.printf(">%3d|%3d|%3d|%3d|%3d|%3d",
                    static_cast<int>(roundf(dummy.currentJoints.a[0])), (int) roundf(dummy.currentJoints.a[1]),
        ...
        oled.setDrawColor(1);
        oled.setCursor(0, 45);
        oled.printf("[%s]:",dummy.GetWifiMode().c_str());
        oled.setCursor(0, 56);
        oled.printf("[ABC]:");
        ...
        oled.setCursor(0, 78);
        count++;
        if (count % 50 ==0 && !dummy.isSendPos()) {
            dummy.Heartbeat();
            count=0;
        }
        if (dummy.isSendPos() && count % 5 == 0) {
            dummy.SendPosJ();
        }
        if (dummy.IsEnabled())
        {
