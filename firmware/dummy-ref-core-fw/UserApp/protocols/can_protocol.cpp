    ...
    if (canCtx->handle->Instance == CAN1)
    {
        ...
        switch (cmd)
        {
            ...
            case 0x25:
                ...
                break;
            case 0x26:
                memcpy(&dummy.motorJ[id]->kp, data, sizeof(uint32_t));//(uint32_t) (data);
                break;
            case 0x27:
                memcpy(&dummy.motorJ[id]->ki, data, sizeof(uint32_t));//(uint32_t) (data);
                break;
            case 0x28:
                memcpy(&dummy.motorJ[id]->kd, data, sizeof(uint32_t));//(uint32_t) (data);
                break;
            case 0x29:
                memcpy(&dummy.motorJ[id]->kv, data, sizeof(uint32_t));//(uint32_t) (data);
            break;
            default:
                break;
        }
